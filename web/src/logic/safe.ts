...
export const buildUpdateFallbackHandler= async(safeAddress: string, module: string): Promise<BaseTransaction> => {
    const safe = await getSafe(safeAddress)
    return {
        to: safeAddress,
        value: "0",
        data: (await safe.setFallbackHandler.populateTransaction(module)).data
    }
} 
